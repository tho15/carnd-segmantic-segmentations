...
def load_vgg(sess, vgg_path):
    ...
    il = graph.get_tensor_by_name(vgg_input_tensor_name)
    kp_out = graph.get_tensor_by_name(vgg_keep_prob_tensor_name)
    ...
    return il, kp_out, l3_out, l4_out, l7_out
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    # replace tf.random_normal_initializer(stddev=0.01) with xavier initializer
    ...
    l4_1x1conv = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding="same",
							kernel_initializer= tf.random_normal_initializer(stddev=0.01),
							kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    output = tf.add(output, l4_1x1conv)
    ...
    l3_1x1conv = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding="same",
							kernel_initializer= tf.random_normal_initializer(stddev=0.01),
							kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    output = tf.add(output, l3_1x1conv)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        l_output = layers(l3_out, l4_out, l7_out, num_classes)
        logits, train_op, cross_entropy_loss = optimize(l_output, correct_label, learning_rate, num_classes)
